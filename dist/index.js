// src/lib/self-check/index.ts
var { default: fs} = (()=>({}));

// src/helpers/utils/index.ts
var {spawn} = (()=>({}));

// src/helpers/env/index.ts
var getEvnVariable = () => {
  return process.env;
};
...
// src/helpers/utils/index.ts
var __dirname = "/Users/aram/Documents/projects/packaholic/src/helpers/utils";
var isLocalPath = (path) => {
  return path.startsWith("./") || path.startsWith("../") || path.startsWith("/") || path.startsWith("~");
};
var defaultExec = (data) => console.log(data.toString());
var exec = ({
  cmd,
  cwd,
...
var takeProjectCacheRoot = () => {
  const { PROJECTS_CACHE_ROOT } = getEvnVariable();
  return q.resolve(__dirname, "../../../", PROJECTS_CACHE_ROOT);
};
...
// src/lib/load-config/index.ts
var { default: fs2} = (()=>({}));
var CONFIG_NAME = "packaholic.config";
var loadConfig = async (root) => {
  const tsConfigPath = q.join(root, `${CONFIG_NAME}.ts`);
  const tsConfig = fs2.existsSync(tsConfigPath);
  if (!tsConfig) {
    throw new MissingConfigError("Config file not found in root directory, check packaholic.config.ts");
  }
  return new Promise((resolve, reject) => {
    import(tsConfigPath).then((config) => {
...
var getGitRepository = async (source) => {
  const root = takeProjectCacheRoot();
  try {
    await exec({
...
var runner = async (options) => {
  const { source } = options;
  let projectRoot = source;
  if (isLocalPath(source)) {
    projectRoot = q.resolve(process.cwd(), source);
  } else {
...
// src/lib/define-config/index.ts
